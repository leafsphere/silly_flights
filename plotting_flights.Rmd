...
| date_recorded | `Date` | date the price was "observed" (in the format `mm-dd-yyyy`) |
...
d <- d %>% 
  filter(sun_country == 'no') %>%
    mutate(label = paste0('Date recorded: ', date_recorded,
...
Oof, this is a lot of code. I'm going to try to make this as interactive as possible, so I'm using the `plotly` package to make this graph. I'm going to plot the lowest price for each day against the number of days before the flight. I'm also going to include a few purple horizontal lines ($120, $140, and $160) to indicate the price range that I'm willing to pay for a one-way ticket. 
...
g <- (d %>%
        filter(!is.na(lowest_price)) %>%
        ggplot(aes(group = 1)) +
        geom_hline(yintercept = c(120,140,160), linetype = "longdash", color = "purple") +
        geom_text(aes(x = 10.5, y = 122, label = scales::dollar(120)), 
                  colour="purple", text = element_text(size=11), show.legend = F) +
        geom_text(aes(x = 10.5, y = 142, label = scales::dollar(140)), 
                  colour="purple", text = element_text(size=11), show.legend = F) +
        geom_text(aes(x = 10.5, y = 162, label = scales::dollar(160)), 
                  colour="purple", text = element_text(size=11), show.legend = F) +
        geom_vline(xintercept = 1:15 * 7, linetype = 'longdash', color = 'grey') +
        geom_line(aes(x = days_before, y = lowest_price, color = flight_date, 
                      text = label, linetype = flight_date), size = 1) +
        ...
        scale_y_continuous(breaks = seq(125, 225, 25),
                           ...
                           limits = c(NA, 235)) +
        geom_point(data = nomids, aes(x = days_before, y = lowest_price, text = label,
  ...
  layout(legend = list(title = list(text = "Flight Date")),
         title = list(text = "Flight Prices vs. Weeks Before Departure",
                      x = 0.5, y = 0.96),
         xaxis = list(title = 'Weeks Before Departure',
...
recorded <- (d %>%
  ...
  ggplot(aes(group = 1)) +
    ...
    scale_y_continuous(breaks = seq(125, 225, 25),
                       labels = scales::dollar_format()) +
    geom_point(data = nomids, aes(x = date_recorded, y = lowest_price, text = label,
                                  color = flight_date, shape = flight_date), size = 2.5) +
    scale_shape_manual(values = c(16,17,18)) +
    theme_minimal()) %>%
  ggplotly(
...
```{r}
mar18 <- d[d$flight_date == '03-18-2024' & !is.na(d$lowest_price), 'lowest_price']
mar25 <- d[d$flight_date == '03-25-2024' & !is.na(d$lowest_price), 'lowest_price']
apr08 <- d[d$flight_date == '04-08-2024' & !is.na(d$lowest_price), 'lowest_price']
dates <- d[d$flight_date == '04-08-2024' & !is.na(d$lowest_price), 'date_recorded']
